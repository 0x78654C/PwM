...
internal sealed class PackageInstaller : IDisposable
{
    ...
    public PackageInstaller(string installation)
    {
        ...
        _workspace = Path.Combine(_installation, ".pwm-update-" + Guid.NewGuid().ToString("N"));
    ...
    public static void ValidateInstallation(string installation)
    {
        ...
        if (Path.TrimEndingDirectorySeparator(path).Equals(Path.TrimEndingDirectorySeparator(Path.GetPathRoot(path)), StringComparison.OrdinalIgnoreCase)
            || !File.Exists(Path.Combine(path, "PwM.exe")))
            throw new InvalidDataException("Choose an existing PwM installation folder containing PwM.exe.");
        RejectReparsePoints(path);
        RejectReparsePoints(Path.Combine(path, "PwM.exe"));
    }
    ...
    internal static string ResolveEntry(string root, string entry)
    {
        ...
        if (parts.Length == 0 || parts.Any(p => string.IsNullOrWhiteSpace(p) || p == "." || p == ".."
            ...
            || p.StartsWith(".pwm-update-", StringComparison.OrdinalIgnoreCase)
    ...
    public string Extract(string zipPath, CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(Path.Combine(_stage, "PwM.exe"))) return _stage;
        var roots = Directory.GetDirectories(_stage);
        if (roots.Length == 1 && Directory.GetFiles(_stage).Length == 0 && File.Exists(Path.Combine(roots[0], "PwM.exe")))
            return roots[0];
        throw new InvalidDataException("The update ZIP must contain PwM.exe at its root or in a single enclosing folder.");
    }

    public static void ValidatePayload(string root, UpdateOptions options)
    {
        if (options.Architecture != "x64")
            throw new InvalidDataException("Automatic updates support Windows x64 installations.");
        string assembly = Path.Combine(root, "PwM.dll");
        if (!File.Exists(assembly) || !File.Exists(Path.Combine(root, "PwM.runtimeconfig.json")))
            throw new InvalidDataException("The update is missing PwM.dll or PwM.runtimeconfig.json.");
        ...
        // The apphost machine identifies the package's process architecture, even for AnyCPU PwM.dll.
        using var executable = new BinaryReader(File.OpenRead(Path.Combine(root, "PwM.exe")));
        if (executable.ReadUInt16() != 0x5A4D) throw new InvalidDataException("PwM.exe is not a Windows executable.");
        executable.BaseStream.Position = 0x3c;
        int offset = executable.ReadInt32();
        if (offset < 64 || offset > executable.BaseStream.Length - 6) throw new InvalidDataException("PwM.exe has an invalid executable header.");
        executable.BaseStream.Position = offset;
        if (executable.ReadUInt32() != 0x4550 || executable.ReadUInt16() != 0x8664)
            throw new InvalidDataException("The update architecture does not match this PwM installation.");
    }
    ...
    public void Install(string payload, IProgress<(int Percent, string File)> progress)
    {
        ...
        {
            if (Path.GetExtension(file.Relative).Equals(".x", StringComparison.OrdinalIgnoreCase)
                || Path.GetFileName(file.Relative).Equals("PwM.Json", StringComparison.OrdinalIgnoreCase)
                || Path.GetFileName(file.Relative).Equals("lockedUser", StringComparison.OrdinalIgnoreCase))
                throw new InvalidDataException("The update ZIP must not contain vaults or personal vault settings.");
        ...
        catch (Exception installError)
        {
            ...
            throw new IOException("The update could not be installed. The previous PwM files were restored. " + installError.Message, installError);
        }
    }
    ...
    public void Dispose()
    {
        ...
        try
        {
            if (Path.GetDirectoryName(_workspace).Equals(_installation, StringComparison.OrdinalIgnoreCase)
                && Path.GetFileName(_workspace).StartsWith(".pwm-update-", StringComparison.Ordinal)
