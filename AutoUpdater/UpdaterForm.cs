...
internal sealed class UpdaterForm : UpdateWindow
{
    ...
    public UpdaterForm(UpdateOptions options, Func<System.Net.Http.HttpClient> createClient = null)
    {
        ...
        if (_parent.StartTime.ToUniversalTime().Ticks != options.ParentStart
            || !Path.GetFullPath(_parent.MainModule.FileName).Equals(Path.Combine(options.InstallDirectory, "PwM.exe"), StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException("The PwM process does not match this update request.");
        Heading.Text = "Updating PwM";
        Summary.Text = "We’ll download the release, verify it, and install it. Your vaults and settings stay in place.";
    ...
    private async Task RunUpdateAsync()
    {
        ...
        string zip = Path.Combine(downloadDirectory, "PwM-package-" + Guid.NewGuid().ToString("N") + ".zip");
        try
        {
            ...
            var release = ReleaseCatalog.SelectUpdate(releases, _options.CurrentVersion, _options.Architecture, _options.Version)
                ?? throw new InvalidOperationException("This release is no longer available. Reopen PwM and check for updates again.");
            ShowRelease(_options.CurrentVersion, release);
            Summary.Text = $"Downloading {release.Package.Name}. The verified ZIP will replace the files in your current PwM installation.";
            ShowProgress("2 / 4   Downloading PwM", 0);
            var download = new Progress<DownloadProgress>(p => ShowProgress("2 / 4   Downloading PwM", p.Percent, p.Description));
            ...
            ShowProgress("Waiting for PwM to close", 0, "Close PwM to continue.");
            ...
            Summary.Text = "Installing the verified release. PwM will be ready to reopen in a moment.";
            var install = new Progress<(int Percent, string File)>(p => ShowProgress("4 / 4   Installing PwM", p.Percent, p.File));
            await Task.Run(() => installer.Install(payload, install));
            installed = true;
            Heading.Text = "You’re up to date";
            Summary.Text = $"PwM {_options.Version} is installed and ready to use.";
            ShowProgress("Update complete", 100, "Your vaults and settings are preserved.");
        }
        catch (OperationCanceledException)
        {
            Heading.Text = "Update cancelled";
            Summary.Text = "PwM’s installed files were not changed. You can try again from the Check for updates menu.";
        ...
        catch (Exception ex)
        {
            ...
            Summary.Text = "Review the details below, then try again from PwM’s Check for updates menu.";
            Notes.Text = ex.Message;
            ShowProgress("Update stopped", 0, "Close other PwM windows and check that its folder is writable.");
        }
        finally
        {
            _installing = false;
    ...
    {
        foreach (var process in Process.GetProcessesByName("PwM"))
        {
            using (process)
            {
                try
                {
                    ...
                    if (Path.GetFullPath(process.MainModule.FileName).Equals(Path.Combine(_options.InstallDirectory, "PwM.exe"), StringComparison.OrdinalIgnoreCase))
                        throw new IOException("Another PwM window is using this installation. Close it and try the update again.");
                }
    ...
    private void Restart()
    {
        try
        {
            Process.Start(new ProcessStartInfo(Path.Combine(_options.InstallDirectory, "PwM.exe"))
            { WorkingDirectory = _options.InstallDirectory, UseShellExecute = true });
            Close();
        }
        catch (Exception ex) { MessageBox.Show(this, ex.Message, "Couldn’t start PwM", MessageBoxButtons.OK, MessageBoxIcon.Error); }
    }
