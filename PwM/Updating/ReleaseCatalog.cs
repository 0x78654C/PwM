...
{
    public const string ReleasesUrl = "https://github.com/0x78654C/PwM/releases";
    private static readonly Regex PackageName = new(
        @"\APwM-v(?<version>\d+\.\d+\.\d+(?:\.\d+)?)_(?<arch>x64)_R2R\.zip\z",
    ...
    {
        if (architecture != "x64")
            throw new NotSupportedException("Automatic updates support Windows x64 installations.");
    ...
    public static Uri GetDownloadUri(ReleaseAsset asset)
    {
        if (!Uri.TryCreate(asset.DownloadUrl, UriKind.Absolute, out var uri)
            ...
            || !uri.AbsolutePath.StartsWith("/0x78654C/PwM/releases/download/", StringComparison.OrdinalIgnoreCase)
            || !Uri.UnescapeDataString(uri.Segments.Last()).Equals(asset.Name, StringComparison.Ordinal)
            || uri.Query.Length != 0 || uri.Fragment.Length != 0)
            throw new InvalidDataException("The download is not a PwM release asset on GitHub.");
        return uri;
    }
    ...
    public static HttpClient CreateClient()
    {
        ...
        client.DefaultRequestHeaders.UserAgent.ParseAdd("PwM-Updater/1.0");
    ...
    public static async Task<List<GitHubRelease>> ReadAsync(HttpClient client, CancellationToken cancellationToken)
    {
        ...
        for (int page = 1; ; page++)
        {
            using var response = await client.GetAsync(
                $"https://api.github.com/repos/0x78654C/PwM/releases?per_page=100&page={page}", cancellationToken).ConfigureAwait(false);
