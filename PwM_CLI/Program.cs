...
";
        ...
        private static void AddPasswords()
        {
            ...
            string encryptdata = AES.Encrypt(decryptVault + "\n" + System.Text.Json.JsonSerializer.Serialize(keyValues), masterPassword);
        ...
        private static void ReadPass()
        {
            ...
            using (var reader = new StringReader(decryptVault))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    {
                        var outJson = System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, string>>(line);
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (line.Length > 0)
                    {
                        ...
                        if (outJson["site/application"] == application && outJson["account"] == accountName)
                        {
                            ...
                            accountCheck = true;
                            listApps.Add(System.Text.Json.JsonSerializer.Serialize(keyValues));
                        }
                        else
                        {
                            listApps.Add(line);
